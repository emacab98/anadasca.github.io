...
async function populateComments() {
  ...
  for (var i = 0; i < posts.length; i++) {
    if (posts[i].table.major_element.id == element_id) {
      index = i;
      break;
    }
  }
  ...
  var found_element = posts[index].table;
  ...
  for (var i = 0; i < len; i++) {
    ...
    for (var j = 0; j < upvoted_len; j++) {
      ...
      if (element[j].comment_id == found_element.comments[i].table.comment_id) {
    ...
    username.innerHTML = "<b> "+ found_element.comments[i].table.user + "</b>";
...
function back_to(){
  ...
  else if(element == "what_if") window.location.href= "../What-If/What_ifs.html"
